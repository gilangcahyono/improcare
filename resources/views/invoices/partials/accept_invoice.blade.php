...
@foreach ($invoices as $invoice)
  <div
    ...
    id="accept-invoice-modal{{ $invoice->id }}">
      ...
      <div class="relative rounded-lg bg-white shadow dark:bg-gray-800">
        ...
        <div class="flex justify-end p-2">
          <button type="button"
            ...
            data-modal-hide="accept-invoice-modal{{ $invoice->id }}">
        ...
        <div class="p-6 pt-0 text-center">
          ...
          <h3 class="mb-6 mt-5 text-lg text-gray-500 dark:text-gray-400">Anda yakin ingin menyetujui
            {{ $invoice->name }} ?</h3>
          <form action="{{ route('invoices.update', ['invoice' => $invoice->id]) }}" class="inline" method="POST">
            @method('PUT')
            @csrf
            <input type="hidden" name="approved" value="{{ true }}">
            <input type="hidden" name="done" value="{{ true }}">
            <button type="submit"
              class="mr-2 inline-flex items-center rounded-lg bg-blue-600 px-3 py-2.5 text-center text-base font-medium text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800">
              Konfirmasi
            </button>
          </form>
          <a href="#"
