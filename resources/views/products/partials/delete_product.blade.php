    ...
    <div class="relative h-full w-full max-w-md px-4 md:h-auto">
      ...
      <div class="relative rounded-lg bg-white shadow dark:bg-gray-800">
        ...
        <div class="p-6 pt-0 text-center">
          ...
          <h3 class="mb-6 mt-5 text-lg text-gray-500 dark:text-gray-400">Anda yakin ingin menghapus produk
            {{ $product->name }}
            ?</h3>
          <form action="{{ route('products.destroy', ['product' => $product->id]) }}" class="inline" method="POST">
            @method('DELETE')
            @csrf
            <button type="submit"
